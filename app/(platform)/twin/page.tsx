  ...
  return (
    <>
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-2">
        <Card>
          <CardHeader title="Live sensor network" right={<span className="text-xs text-fg-faint">Streaming via IoT gateway</span>} />
        ...
        <Card className="self-start">
          <CardHeader title="Reality capture" right={<span className="text-xs text-fg-faint">Compared against design model automatically</span>} />
          <div className="divide-y divide-edge">
            {twinCaptures.map((c) => (
              <div key={c.kind} className="px-5 py-3.5">
                <div className="flex items-center justify-between">
                  <div className="text-sm font-medium text-fg">{c.kind}</div>
                  <span className="text-xs text-fg-faint">{c.date}</span>
                </div>
                <div className="mt-0.5 text-xs text-fg-faint">
          ...
          <div className="border-t border-edge px-5 py-4 text-sm text-fg-faint">
